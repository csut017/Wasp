...
namespace Wasp.Core.Data
{
    ...
    /// </summary>
    public class RosterPackage
    {
        private RosterPackage(PackageSettings settings)
        {
        ...
        /// <returns>The new <see cref="RosterPackage"/> instance.</returns>
        public static async Task<RosterPackage> LoadAsync(Stream stream, PackageSettings settings)
        {
            var package = new RosterPackage(settings);
            if (!settings.IsCompressed)
            {
                package.Roster = await settings.Format.DeserializeRosterAsync(stream);
            }
            else
            {
                ...
                package.Roster = await settings.Format.DeserializeRosterAsync(zipStream);
            }
        ...
        /// <param name="settings">The <see cref="PackageSettings"/> instance to use.</param>
        /// <returns>The new <see cref="RosterPackage"/> instance.</returns>
        public static async Task<RosterPackage> LoadAsync(string path, PackageSettings? settings = null)
        {
        ...
        /// <summary>
        /// Starts a new <see cref="RosterPackage"/> with the default settings.
        /// </summary>
        /// <returns>The new <see cref="RosterPackage"/> instance.</returns>
        public static RosterPackage New()
        {
            return new RosterPackage(new PackageSettings());
        }
        ...
        public async Task SaveAsync(Stream stream)
        {
            ...
            if (!this.Settings.IsCompressed)
            {
                ...
                await this.Settings.Format.SerializeRosterAsync(this.Roster, stream);
                return;
            }
            ...
            using var zipStream = entry.Open();
            await this.Settings.Format.SerializeRosterAsync(this.Roster, zipStream);
        }
    }
}